...
import { editTool, getCategories, getPricingPlan, getTags, getToolByid, getTools } from "../../../../../../constraint/api/auth.route";
...
const API_ENDPOINTS = {
  ...
  EDIT_TOOL: getToolByid,
  EDIT_TOOL_PUT: getTools,
};

const EditTool = () => {
  const { id } = useParams(); // Extract tool ID from URL (e.g., /tools/edit/1)
  ...
  // Fetch tags, categories, pricing plans, and tool data
  const fetchData = async () => {
    dispatch({ type: FETCH_ACTION_STATE.FETCH_START });
    try {
      const [tagsResponse, categoriesResponse, pricingPlansResponse, toolResponse] =
        await Promise.all([
          axios.get(API_ENDPOINTS.TAGS),
          axios.get(API_ENDPOINTS.CATEGORIES),
          axios.get(API_ENDPOINTS.PRICING_PLANS),
          axios.get(`${API_ENDPOINTS.EDIT_TOOL}/${id}`), // Fetch existing tool data
        ]);
      const tool = toolResponse.data?.data || null;
      dispatch({
      ...
      if (tool) {
        ...
        setFormData({
          ...
          pricing_plan_id: tool.pricing_plan_id?.toString() || "",
          github_uri: tool.github_uri || "",
          youtube_uri: tool.youtube_uri || "",
          x_uri: tool.x_uri || "",
          facebook_uri: tool.facebook_uri || "",
          instagram_uri: tool.instagram_uri || "",
          categoryId: tool.categoryId?.toString() || "",
          tagIds: tool.tagIds?.map((id) => id.toString()) || [],
        });
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    data.append("id", id); // Include tool ID for update
    ...
    if (image) data.append("image", image); // Only append if a new image is selected
    if (backgroundImage) data.append("background_image", backgroundImage); // Only append if a new background image is selected

    try {
      const response = await axios.put(`${API_ENDPOINTS.EDIT_TOOL_PUT}/${id}`, data, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      toast.success(response.data.message || "Tool updated successfully.");
      router.push("/tools");
    } catch (error) {
      toast.error(
  ...
  return (
    ...
    >
      <h3 className="font-medium capitalize text-xl mb-4">Edit Tool</h3>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          {toolData?.image && !image && (
            <p className="text-sm text-gray-500">Current image: {toolData.image}</p>
          )}
        </div>
        <div>
          ...
          />
          {toolData?.background_image && !backgroundImage && (
            <p className="text-sm text-gray-500">Current background image: {toolData.background_image}</p>
          )}
        </div>
